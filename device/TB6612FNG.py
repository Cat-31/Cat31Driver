...
class TB6612FNG(object):
    def __init__(self, config):
        if 'ch_a' in config:
            ...
            self.setup_gpio(self, self.ch_a_input, [GPIO.LOW, GPIO.LOW])
        ...
        if 'ch_b' in config:
            ...
            self.setup_gpio(self, self.ch_b_input, [GPIO.LOW, GPIO.LOW])
            ...
            if 'pwm' in config['ch_b']:
                ...
                self.pwm_b.start(0)
        GPIO.setup(config['stby'], GPIO.OUT, initial=GPIO.HIGH)

    def ch_a_start_up(self, dutycycle):
        self.set_gpio(self, self.ch_a_input, [GPIO.LOW, GPIO.HIGH])
        self.pwm_a.ChangeDutyCycle(dutycycle)

    def ch_a_reverse(self, dutycycle):
        self.set_gpio(self, self.ch_a_input, [GPIO.HIGH, GPIO.LOW])
        self.pwm_a.ChangeDutyCycle(dutycycle)

    def ch_b_start_up(self, dutycycle):
        self.set_gpio(self, self.ch_b_input, [GPIO.LOW, GPIO.HIGH])
        self.pwm_b.ChangeDutyCycle(dutycycle)

    def ch_b_reverse(self, dutycycle):
        self.set_gpio(self, self.ch_b_input, [GPIO.HIGH, GPIO.LOW])
        self.pwm_b.ChangeDutyCycle(dutycycle)

    def ch_a_stop(self):
        self.set_gpio(self, self.ch_a_input, [GPIO.LOW, GPIO.LOW])
        self.pwm_a.ChangeDutyCycle(0)

    def ch_b_stop(self):
        self.set_gpio(self, self.ch_b_input, [GPIO.LOW, GPIO.LOW])
